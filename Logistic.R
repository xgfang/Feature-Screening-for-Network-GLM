...
U[which(U<1)] <- -1
beta[[1]] <- rep(0,p)
beta[[1]][M[[1]]] <- U*(4*log(n)/sqrt(n)+abs(rnorm(8))/4)

M[[2]] <- c(1,3,5,7,9)
beta[[2]] <- rep(0,p)
beta[[2]][M[[2]]] <- c(0.8, -0.75, 0.7, -0.65, 0.6)
# beta[[2]][M[[2]]] <- c(1.6, -1.5, 1.4, -1.3, 1.2)
...
names2[[2]] <- c("NW-SIS","SMLE","PMLE_NPG","PMLE_AMPG","PMLE_ANPG")
...
for(i in 1:S){
  ...
  for(j in 1:3){
    ...
    ptm1 <- proc.time()
    fit13_11 <- PMLE_NPG(X, Y, W, family, theta_initial, k, tol=1e-4, maxiter=500)
    ptm2 <- proc.time()
    ...
    fit13_12 <- PMLE_POST(X, Y, W, p, fit13_11$ix,family,tol=1e-4, maxiter=1000)
...
# write.table(table1,paste("Logistic-","Screening-",n,"-",p,".txt",sep=""))
# write.csv(table1,paste("Logistic-","Screening-",n,"-",p,".csv",sep=""))

res2 <- apply(result2,1:3,mean)
table2 <- round(rbind(res2[1,,],res2[2,,],res2[3,,]),3)
# write.table(table2,paste("Logistic-","Estimation-",n,"-",p,".txt",sep=""))
# write.csv(table2,paste("Logistic-","Estimation-",n,"-",p,".csv",sep=""))

table1
table2


# problem: the performance of algorithm is accepatable, but not the post screening part
# the post screening has no improvement but make it worse