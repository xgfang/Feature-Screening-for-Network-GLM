...
U[which(U<1)] <- -1
beta[[1]] <- rep(0,p)
beta[[1]][M[[1]]] <- U*(4*log(n)/sqrt(n)+abs(rnorm(8)))
...
write.table(table1,paste("Linear-","Screening-",n,"-",p,".txt",sep=""))
write.csv(table1,paste("Linear-","Screening-",n,"-",p,".csv",sep=""))

res2 <- apply(result2,1:3,mean)
table2 <- round(rbind(res2[1,,],res2[2,,],res2[3,,]),3)
write.table(table2,paste("Linear-","Estimation-",n,"-",p,".txt",sep=""))
write.csv(table2,paste("Linear-","Estimation-",n,"-",p,".csv",sep=""))
